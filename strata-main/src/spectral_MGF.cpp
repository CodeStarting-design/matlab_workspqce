...
void SpectralMGF::ComputeKmi(std::array<std::complex<double>, 5> &K)
{
	...
	std::ofstream logFile("ComputeKmi_log.txt", std::ios::app); // Open file in append mode
	if (logFile.is_open())
	{
		logFile << "TIe: " << TIe << ", TIh: " << TIh << ", ";
		logFile << "GVe_ii: " << GVe_ii << ", GVh_ii: " << GVh_ii << ", ";
		logFile << "GIe_ii: " << GIe_ii << ", GIh_ii: " << GIh_ii << ", ";
		logFile << "ki: " << ki << ", krhosq: " << krhosq << ", ";
		logFile << "kzm: " << kzm << ", km: " << km << ", ";
		logFile << "K[3]: " << K[3] << ", K[2]: " << K[2] << std::endl;
		logFile.close();
	}
